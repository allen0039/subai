...
## Confirm-only email replacement — 2026-09-15

Targeted backend service/handler tests passed, including explicit confirmation, password preservation, email/alias collisions and legacy first binding. Frontend component/locale tests (19), typecheck, lint and production build passed; confirmation text correction passed 16 component tests. Backend lint: 0 issues. Isolated browser end-to-end verified old/new email text, cancellation without submission, confirmed replacement, unchanged-password login with the new email, and rejection of the old login. No JavaScript errors. Test containers, volumes and temporary credentials were removed. Production image b372526 was published and deployed successfully; health/login return 200, removed APIs 404, unauthenticated admin access 401. Production authenticated testing was not performed because initial admin credentials are stale; no production email or password was changed.
