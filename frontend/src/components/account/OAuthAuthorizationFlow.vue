...
const oauthAuthCodeDesc = computed(() => props.platform === 'openai' ? t('admin.accounts.oauth.openai.callbackUrlDesc') : t(getOAuthKey('authCodeDesc')))
const oauthAuthCode = computed(() => props.platform === 'openai' ? t('admin.accounts.oauth.openai.callbackUrlLabel') : t(getOAuthKey('authCode')))
const oauthAuthCodePlaceholder = computed(() => props.platform === 'openai' ? 'http://localhost:1455/auth/callback?code=…&state=…' : t(getOAuthKey('authCodePlaceholder')))
const oauthAuthCodeHint = computed(() => props.platform === 'openai' ? t('admin.accounts.oauth.openai.callbackUrlDesc') : t(getOAuthKey('authCodeHint')))
...
watch(authCodeInput, (newVal) => {
  if (props.platform !== 'gemini' && props.platform !== 'antigravity' && props.platform !== 'grok') return
  ...
  if (trimmed.includes('code=')) {
    try {
      ...
      if ((props.platform === 'gemini' || props.platform === 'antigravity' || props.platform === 'grok') && stateParam) {
      ...
      if ((props.platform === 'gemini' || props.platform === 'antigravity' || props.platform === 'grok') && stateMatch && stateMatch[1]) {
