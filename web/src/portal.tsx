...
const SubscriptionCard: React.FC<{ sub: any }> = ({ sub }) => <div className="subscription-card">
  <div><span className="eyebrow">{sub.service_group?.name ? `服务分组 · ${sub.service_group.name}` : `订阅版本 ${sub.plan_version}`}</span><h3>{sub.plan_name}</h3></div><Badge value={sub.status} domain="subscription"/>
...
export const AdminPlans: React.FC = () => {
  const emptyForm = {name:"",description:"",daily_limit_usd:"",weekly_limit_usd:"",monthly_limit_usd:"",rate_multiplier:"1.00",concurrency_limit:"1",max_keys:"",default_validity_days:"30",service_group_id:"",model_pricing:[]};
  ...
  const load = useCallback(async () => {
    try {
      const [planResult,groupResult] = await Promise.all([api.get<any>("/api/admin/plans"),api.get<any>("/api/admin/service-groups")]);
      setPlans(planResult.data ?? []);
      setGroups((groupResult.data ?? []).filter((group:any) => group.status === "active" && group.subscription_type === "subscription"));
      setError("");
  ...
  const openCreate = () => { setEditing(null); setForm({...emptyForm,service_group_id:groups[0]?.id ?? "",model_pricing:[]}); setShow(true); };
  const openEdit = (plan:any) => {
    ...
    setForm({
      ...
      service_group_id:plan.service_group?.id ?? "",
  ...
  return <div className="portal">
    ...
    <p className="muted">套餐只负责订阅期限、配额、共享并发和密钥数量；上游账号、模型同步、映射与白名单统一在“服务分组”管理。</p>
    {error && <div className="error">{error}</div>}
    <table className="tbl"><thead><tr><th>套餐</th><th>状态</th><th>额度（每日／每周／每月）</th><th>计费倍率</th><th>套餐共享并发</th><th>接口密钥数量</th><th>订阅服务分组</th><th/></tr></thead><tbody>{plans.map(plan => <tr key={plan.id}>
      ...
      <td>{plan.service_group?.name || "历史套餐：待迁移"}{plan.service_group?.platform && <small className="muted"> · {plan.service_group.platform === "codex" ? "Codex" : plan.service_group.platform}</small>}</td>
    ...
    {show && <Modal className="plan-modal" title={editing ? `编辑套餐 · ${editing.name}` : "创建套餐草稿"} onClose={() => { setShow(false); setEditing(null); }} onSubmit={save} submitLabel={editing ? "保存新版本" : "创建草稿"}>
      ...
      <label className="field"><span className="field-label">订阅服务分组</span><select value={form.service_group_id} onChange={e => setForm({...form,service_group_id:e.target.value})} required><option value="">请选择订阅服务分组</option>{groups.map(group => <option key={group.id} value={group.id}>{group.name} · {group.platform === "codex" ? "Codex" : group.platform}</option>)}</select><small className="muted">模型、账号和路由由所选服务分组决定；请先在服务分组完成配置。</small></label>
