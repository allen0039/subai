...
func (s *OpenAIOAuthService) GenerateAuthURL(ctx context.Context, proxyID *int64, redirectURI, platform string, ownerID ...int64) (*OpenAIAuthURLResult, error) {
	...
	// Validate the configured URLs before creating a session.
	for _, raw := range []string{openai.ConfiguredAuthorizeURL(), openai.ConfiguredTokenURL(), redirectURI} {
		u, err := url.Parse(raw)
		if err != nil || u.Host == "" || (u.Scheme != "http" && u.Scheme != "https") || u.User != nil || u.Fragment != "" {
			return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_OAUTH_INVALID_CONFIG", "invalid OAuth endpoint or redirect URI")
		}
	}
	// Store session
	...
	if len(ownerID) > 0 {
		session.OwnerID = ownerID[0]
	}
	if err := s.sessionStore.Set(sessionID, session); err != nil {
		return nil, infraerrors.New(http.StatusServiceUnavailable, "OPENAI_OAUTH_SESSION_STORE_UNAVAILABLE", "OAuth session storage unavailable")
	}
...
func (s *OpenAIOAuthService) ExchangeCode(ctx context.Context, input *OpenAIExchangeCodeInput) (*OpenAITokenInfo, error) {
	if input == nil {
		return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_OAUTH_INVALID_INPUT", "authorization input is required")
	}
	// Get session
	...
	if session.OwnerID != input.OwnerID {
		return nil, infraerrors.New(http.StatusForbidden, "OPENAI_OAUTH_SESSION_OWNER", "authorization belongs to another administrator")
	}
	if input.CallbackURL != "" {
		callback, err := url.Parse(strings.TrimSpace(input.CallbackURL))
		expected, expectedErr := url.Parse(session.RedirectURI)
		if err != nil || expectedErr != nil || callback.User != nil || callback.Fragment != "" || callback.Scheme != expected.Scheme || !strings.EqualFold(callback.Host, expected.Host) || callback.Path != expected.Path {
			return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_OAUTH_INVALID_CALLBACK", "callback URL does not match the configured redirect URI")
		}
		q, err := url.ParseQuery(callback.RawQuery)
		if err != nil || len(q["code"]) != 1 || len(q["state"]) != 1 || q.Get("error") != "" {
			return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_OAUTH_INVALID_CALLBACK", "callback must contain a single code and state and no provider error")
		}
		// Never use a cached UI state in place of the actual callback state.
		input = &OpenAIExchangeCodeInput{SessionID: input.SessionID, OwnerID: input.OwnerID, Code: q.Get("code"), State: q.Get("state"), ProxyID: input.ProxyID, RedirectURI: input.RedirectURI}
	}
	...
	// Authorization and exchange use the same egress proxy.
	proxyURL := session.ProxyURL
	if input.ProxyID != nil && (session.ProxyID == nil || *input.ProxyID != *session.ProxyID) {
		return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_OAUTH_PROXY_CHANGED", "proxy cannot change during authorization")
	}
	...
	if input.RedirectURI != "" {
		if input.RedirectURI != redirectURI {
			return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_OAUTH_INVALID_CALLBACK", "redirect URI cannot change during authorization")
		}
	}
	...
	if !s.sessionStore.TryConsume(input.SessionID) {
		return nil, infraerrors.New(http.StatusConflict, "OPENAI_OAUTH_SESSION_CONSUMED", "authorization already consumed; generate a new authorization URL")
	}
	...
	if tokenResp == nil || strings.TrimSpace(tokenResp.AccessToken) == "" {
		return nil, infraerrors.New(http.StatusBadGateway, "OPENAI_OAUTH_EMPTY_TOKEN", "token response missing access_token")
	}
	// Parse ID token to get user info
	var userInfo *openai.UserInfo
	if tokenResp.IDToken != "" {
		claims, parseErr := openai.ParseSubAIIdentity(tokenResp.IDToken)
		if parseErr != nil {
	...
	tokenInfo := &OpenAITokenInfo{
		ChatGPTAccountID: tokenResp.AccountID,
...
func (s *OpenAIOAuthService) RefreshAccountToken(ctx context.Context, account *Account) (*OpenAITokenInfo, error) {
	...
	clientID := account.GetCredential("client_id")
	fresh, err := s.RefreshTokenWithClientID(ctx, refreshToken, proxyURL, clientID)
	if err != nil {
		return nil, err
	}
	if fresh.IDToken == "" {
		fresh.IDToken = account.GetCredential("id_token")
	}
	if fresh.ChatGPTAccountID == "" {
		fresh.ChatGPTAccountID = account.GetCredential("chatgpt_account_id")
	}
	if fresh.Email == "" {
		fresh.Email = account.GetCredential("email")
	}
	return fresh, nil
}
...
// WithSessionStore replaces the test/local store before the service is exposed.
