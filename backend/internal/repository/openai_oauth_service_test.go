...
func (s *OpenAIOAuthServiceSuite) TestExchangeCode_DefaultRedirectURI() {
	...
	s.setupServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("originator") != "" || r.Header.Get("User-Agent") != "Go-http-client/1.1" {
			errCh <- "expected SubAI standard HTTP transport"
			w.WriteHeader(http.StatusBadRequest)
			return
		}
...
func (s *OpenAIOAuthServiceSuite) TestNonSuccessStatus_DoesNotExposeBody() {
	...
	require.NotContains(s.T(), err.Error(), "body:")
}
